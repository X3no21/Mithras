...
class SweetSpotFinder:
    def __init__(self, config, hooker=None, node_lifter=None):
        apk_path = config['smartphone_apk_path']
    ...
    def get_constructor_method(self, method):
        try:
            ...
            logger.error("Cant find constructor")
            return []
    ...
    def get_function_setter(self, inp, filter_method):
        ...
        for st in first_block.statements:
            if hasattr(st, 'right_op') and type(st.right_op) == soot_wrapper.sootir.soot_value.SootParamRef:
    ...
    def get_pysoot_method(self, method):
        if type(method) == soot_wrapper.sootir.soot_method.SootMethod:
            return method
        try:
            ...
            logger.error("Class not found in class hierarchy")
            return None
                    ...
                    try:
                        while v_ass is None:
                            current_var = [s.right_op for b in tainted_blocks for s in b.statements if
                                     type(s) == soot_wrapper.sootir.soot_statement.AssignStmt and hasattr(s.left_op, 'name') and
                                     s.left_op.name == current_var.name]
                            current_var = current_var[0]
                            if hasattr(current_var, 'field'):
                                v_ass = current_var
                    except:
                        logger.error("Could not find variable assignment")
                        continue
                ...
                while True:
                    ...
                    for info in methods_instances:
                        ...
                        if method:
                            ...
                            logger.warning("Found instance, but not the method declaration.. check me.")
                            continue
    ...
    def get_obj_class_returned(self, method):
        ...
        this_asn = [s for s in stmts if type(s) == soot_wrapper.sootir.soot_statement.IdentityStmt and
        ...
        var_rets = [s for b in pysoot_method.blocks for s in b.statements
                    if type(s) == soot_wrapper.sootir.soot_statement.ReturnStmt
        ...
        for r in var_rets:
            ...
            if r.value.name in fslicer.tainted_in_method(pysoot_method):
                ...
                fields += [s.right_op.field for s in affected_stmts if hasattr(s, 'right_op') and hasattr(s.right_op, 'base')
                            ...
                            and type(s.right_op) == soot_wrapper.sootir.soot_value.SootInstanceFieldRef]
                ...
                fields += [s.left_op.field for s in affected_stmts if hasattr(s, 'left_op') and hasattr(s.left_op, 'base')
                             ...
                             and type(s.left_op) == soot_wrapper.sootir.soot_value.SootInstanceFieldRef]
        return fields
    ...
    def get_referred_obj_fields(self, sender):
        ...
        # returned from calls
        calls_stms = [s for s in stmts if type(s) == soot_wrapper.sootir.soot_statement.AssignStmt
                      and type(s.right_op) == soot_wrapper.sootir.soot_expr.SootVirtualInvokeExpr
        ...
        while worklist:
            ...
            while caller_worklist:
                ...
                try:
                    ...
                    logger.error("Apk exploded: " + str(ex))
                    logger.error("Skipping this one")
                    continue
    ...
    def start(self, sender, ran_fun=lambda *args: None, lifter=None):
        if not self.lifter:
            ...
            if self.lifter is None:
                ...
                logger.debug("Lifter pickled in " + LIFTER_PICKLE)
