        ...
        Java.perform(function x() {
            ...
            apkHooks.forEach(function(element) {
                ...
                try
                {
                    ...
                    if (clx !== undefined) {
                        if (clx[method] !== undefined) {
                            if (clx[method].overload.apply(clx[method], params) !== undefined) {
                                if (clx[method].overload.apply(clx[method], params).implementation !== undefined) {
                                    clx[method].overload.apply(clx[method], params).implementation = function f()
                                    {
                                        var args = Array.prototype.slice.call(arguments);
                                        recordMethod(cls, method, params, ret);
                                        // we gotta separate params from return value as they might be aliased
                                        // and we want to capture those two different values
                                        if (getInstances)
                                            recordInstanceParams(cls, method, params, arguments);
                                        if (ret !== 'void') {
                                            try {
                                                var to_ret = undefined;
                                                if (this[method] !== undefined) {
                                                    if (this[method].overload.apply(this[method], params) != undefined) {
                                                        to_ret = this[method].overload.apply(this[method], params).apply(this, args);
                                                    }
                                                }

                                                if (to_ret === undefined) {
                                                    to_ret = clx[method].overload.apply(clx[method], params).apply(this, args);
                                                }
                                                ...
                                                console.error("Return error -- " + error.toString() + ' ' + hooking);
                                            }
                                        }
                                    }
                                }
