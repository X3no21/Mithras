...
class ArgFuzzer:
    ...
    def save_run(self):
        logger.info("Interesting run  detected.")
    ...
    def hook_new_methods(self, methods):
        time.sleep(5)
        logger.debug("Hooking {} methods".format(len(methods)))
    ...
    def wait_for_repetition(self):
        while not self.hooker.is_repetition_done():
            ...
            if self.reran_proc.poll() is not None:
                logger.debug("Stop waiting, reran finished. Might not be the correct send")
                return
    ...
    def spawn_and_fuzz_class_field(self, method, times, action, field_name, finfo, fast_fuzz=False, pos_in_trace=0):
        cls = method[0]
        m = method[1]
        ...
        logger.info("Setting up {} different values for the field {}. "
                 "This might take a while".format(str(times), str(field_name)))

        for _ in range(times):
            self.hooker.next_fields_list()
            field_val = self.vals.create_value(obj_type, action, self.hooker.modify_class_field, field_name)
            if field_val is None:
                logger.error("fuzz_primitive has does not have a function for type {}".format(obj_type))
                continue

            key_h = (field_name, obj_type)
            if key_h not in self.fuzz_history:
                self.fuzz_history[key_h] = []
            self.fuzz_history[key_h].append(field_val[0])
        logger.info("Done.")
    ...
    def get_class_fields(self, cls, extended=True):
        ...
        # get all the fields: own and inherited both
    ...
    def spawn_and_fuzz_param(self, method, times, action, pos_to_fuzz, fast_fuzz=False, single_call_fuzz=False,
                             curr_call=0):
        cls = method[0]
        m = method[1]
        params = method[2]
        ...
        logger.info("Setting up {} different values for the {}-th parameters of {}:{}"
                ...
                if i in pos_to_fuzz:
                    if not self.is_known_type(p_type):
                        ...
                        try:
                            ...
                            logger.warning('Cannot find class {}'.format(p_type))
                            continue
                        for fname, finfo in class_fields:
                            obj_type = finfo[1]
                            self.vals.create_value(obj_type, action, self.hooker.modify_class_field, fname)
                            self.hooker.set_arg_simple_obj()
                    else:
                        par_val = self.vals.create_value(p_type, action, self.hooker.create_obj)
                        if par_val is None:
                            logger.error("fuzz_primitive does not have a function for type {}".format(p_type))
                    ...
                    if key_h not in self.fuzz_history:
                        self.fuzz_history[key_h] = []
                    self.fuzz_history[key_h].append(par_val[0])
                else:
                    self.hooker.set_unfuzzed_obj()
    ...
    def spawn_and_replay_class_field(self, *kargs, **kwargs):
        logger.error("Replay class fields is not implemented... yet")
            ...
            for p, p_type in enumerate(params):
                if p in pos_to_fuzz:
                    ...
                    if s_p not in traces:
                        ...
                        p_type = params[p]
                        if not self.is_known_type(p_type):
                            logger.error("Replay non primitive fields not implemented..yet.")
                        else:
                            try:
                            ...
                            if not vals[0]:
                                ...
                                try:
                                    par_val = map(getattr(self.vals, 'str_to_' + str(base_type.replace('.', '_'))),
                                                  vals)
                                except:
                                    import ipdb
                                    ipdb.set_trace()
    ...
    def do_fuzz(self, method, spawn_and_prepare_ran, action, ran_fun, remain_reps, *kargs, **kwargs):
        logger.info("Fuzzing device with {} values".format(str(remain_reps)))
        tot_reps = 0
        unlimited = False
        first_run = True
        ...
        if 'single_call_fuzz' in kwargs and kwargs['single_call_fuzz']:
            ...
            logger.info("Hooked function repeated {} times".format(str(util_method_calls)))
        ...
        while remain_reps > 0 or unlimited:
            try:
                spawn_and_prepare_ran(method, reps, action, *kargs, **kwargs)
                ...
                logger.info("Hooked function repeated {} times".format(str(n_repeated)))
                ...
                logger.info("Function fuzzed {} times.".format(str(tot_reps)))
            except ApkExploded as ae:
                logger.error("App exploded")
                # we replayed the function once and the app exploded
                # let's move on
                if tot_reps <= 1 and self.hooker.get_n_repeated() <= 1:
                    logger.debug("Param or class field make the app crash. Stop fuzzing this one.")
                    break
    ...
    def do_fuzz_params_function(self, method, action, ran_fun, fast_fuzz=False, single_call_fuzz=False):
        cls = method[0]
        m = method[1]
        params = method[2]
        if not fast_fuzz:
            logger.info("Fuzzing params functions: Fuzzing might take a while, fast_fuzz is disabled."
                     "We change values to the sweet spot every time we encounter it")
        else:
            logger.info("Fast fuzz is enabled")
        ...
        if not fuzzed:
            logger.debug("Reran finished and sweet spot was not encountered.")
            # break
        logger.info("Param fuzzed")

    # FIXME: handle single_call_fuzz and curr_call
    def do_fuzz_class_fields(self, method, ran_fun, action, fast_fuzz=False, single_call_fuzz=False):
        ...
        if not fast_fuzz:
            logger.info("Fuzzing might take a while, fast_fuzz is disabled."
                     "We change values to the sweet spot every time we encounter it")
        else:
            logger.info("Fast fuzz is enabled")
        ...
        for fname, ftype_info in class_fields:
            ...
            if not self.is_known_type(type_field):
                logger.error("Field {} is not of a primitive type.. skipping this one. Implement me.".format(fname))
                continue

            fuzzed = self.do_fuzz(method, self.spawn_and_fuzz_class_field, action, ran_fun, fname,
                                  ftype_info, N_FUZZ, fast_fuzz=fast_fuzz)
            if not fuzzed:
                logger.debug("Reran finished and sweet spot was not encoutered.")
                break
            logger.info("Object fuzzed")

    def register_normal_run(self, ran_fun):
        # self.an.start_normal_run()
        # ran_fun()
        # if not self.an.stop_normal_run():
        #    logger.error("Can't save pcap files to verify fuzzing results")
        pass
    ...
    def start(self, method, action, fast_fuzz=False, ran_fun=lambda *args: None, reg_run=True, single_call_fuzz=False,
              lifter=None):
        self.lifter = lifter
        ...
        try:
            ...
            if params and params[0] and self.fuzz_fun_params:
                logger.info("Fuzzing params")
                self.do_fuzz_params_function(method, ran_fun, action, fast_fuzz=fast_fuzz, single_call_fuzz=single_call_fuzz)

            if self.fuzz_class_fields:
                logger.info("Fuzzing class fields")
                self.do_fuzz_class_fields(method, ran_fun, action, fast_fuzz=fast_fuzz,
                                          single_call_fuzz=single_call_fuzz)
        except FuzzTerminate as ft:
            logger.info("Fuzz terminate")

        self.fp.close()
        logger.info("Terminating Fuzzing")
    ...
    def replay_trace(self, trace_file, fast_fuzz=False, ran_fun=lambda *args: None):
        ...
        for s_method, r_info in self.traces.items():
