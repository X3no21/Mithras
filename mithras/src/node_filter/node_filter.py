...
NET_PACKAGES = ['java.net.', 'javax.net.', 'android.net.', 'android.webkit.', 'org.apache.']
WEARABLE_PACKAGES = ['com.google.android.gms.wearable']
...
class NodeFilter:
    def __init__(self, config, lifter=None, lazy_execution=False, reload=False):
        ...
        self.apk_path = config['smartphone_apk_path']
    ...
    def get_methods(self, caller_class, caller_method, stmt):
        ...
        is_wearable = any([n for n in WEARABLE_PACKAGES if re.search(f"^{n}(?!.internal)",
                                                                     callee_cls.lower()) is not None])
    ...
    def start(self, reload=False):
        logger.info("Getting APK border nodes")
        if not reload and self.leaf_pickle and os.path.isfile(self.leaf_pickle):
            internal_struct = pickle.load(open(self.leaf_pickle, "rb"))
            ...
            logger.info("APK leaves pickled in " + pickle_name)
