...
def count_diff(apk1, apk2):
    # Make sure the APKs exist.
    exists(apk1)
    exists(apk2)

    output = os.path.join(tempfile.gettempdir(), 'apkdiff')

    # Check the temporary folder exists & clear it.
    folderExists(output, True)

    # Individual folders for each APK.
    temp1 = os.path.join(output, "1/")
    temp2 = os.path.join(output, "2/")

    folderExists(temp1, True)
    folderExists(temp2, True)

    apktoolit(apk1, temp1)
    apktoolit(apk2, temp2)

    mergesmalifolders(temp1)
    mergesmalifolders(temp2)

    compare(os.path.join(temp1, "smali"), os.path.join(temp2, "smali"), False)
